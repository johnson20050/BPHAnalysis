...
void lbWriteSpecificDecay::fillDescriptions(
    ...
    descriptions.add( "lbWriteSpecificDecay", desc );
   return;
}
...
void lbWriteSpecificDecay::produce( edm::Event& ev,
  ...
  if ( writeOnia     ) write( ev, es, lFull      ,     oniaName , false , false );
  if ( writeLam0     ) write( ev, es, lLam0      ,     Lam0Name , false , false );
  if ( writeTkTk     ) write( ev, es, lTkTk      ,     TkTkName , false , false );
  if ( writeLbToLam0 ) write( ev, es, lLbToLam0  , LbToLam0Name , true  , false );
  if ( writeLbToTkTk ) write( ev, es, lLbToTkTk  , LbToTkTkName , true  , true  );
  return;
}
