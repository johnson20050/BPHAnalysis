...
// my secondary fit result store code.
                const std::vector<RefCountedKinematicParticle>& daughters = ptr->kinematicTree()->finalStateParticles();
                if ( daughters.size() != ptr->daughFull().size() )
                ...
                else
                {
                    ...
                    std::cout << "componentList = \n";
                    for ( const auto& lll : componentList )
                        std::cout << " ( " << lll.first << ", " << lll.second << " );\n";
                    ...
                    for ( const RefCountedKinematicParticle& daughter_ : daughters )
                    {
                        ...
                        std::pair<const reco::Candidate*, std::string> myPair;
                        ...
                        while ( iter != iend )
                        {
                            ...
                            if ( fabs( mom.transverse() - dPt_ ) > ptDiff )
                            {
                                ...
                                componentList_.erase( _iter );
                            }
                        }
                        particleList.insert( std::make_pair(daughter_, myPair) );
                    }
                    // particle name - RefCountedKinematicParticle pair is prepared.
                    if ( particleList.size() != daughters.size() ) std::cout << "-----bug: particleList and daughters don't owns the same size! in write() \n";
                    else if ( totalCharge != 0 ) std::cout << "-----bug: the built particle is not neutral! in write()\n";
                    else
                    {
