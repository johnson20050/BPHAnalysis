#include "BPHAnalysis/testProducer/plugins/testProducer.h"
...
testProducer::testProducer( const edm::ParameterSet& ps )
{
...
void testProducer::fillDescriptions(
    edm::ConfigurationDescriptions& descriptions )
{
    ...
    descriptions.add( "testProducer", desc );
    return;
}
...
void testProducer::analyze( const edm::Event& ev,
...
void testProducer::fill( const edm::Event& ev,
                            const edm::EventSetup& es )
{
...
void testProducer::fillTree( const edm::EventSetup& es )
{
    ...
    if (writeLam0) // {{{
    {
        ...
        for ( unsigned int iLam = 0; iLam < nLam0; ++iLam )
        {
            ...
            lam0Br.refitMom.mass    = cand->composite().momentum().mass();
            lam0Br.refitMom.pt      = cand->composite().momentum().pt();
            lam0Br.refitMom.eta     = cand->composite().momentum().eta();
            lam0Br.refitMom.phi     = cand->composite().momentum().phi();
            lam0Br.refitPos.x       = cand->composite().vertex().position().x();
            lam0Br.refitPos.y       = cand->composite().vertex().position().y();
            lam0Br.refitPos.z       = cand->composite().vertex().position().z();
            lam0Br.refitPos.vtxprob = TMath::Prob( cand->composite().vertex().chi2(), cand->composite().vertex().ndof() );
    ...
    for ( unsigned int iLamb = 0; iLamb < nLamb; ++iLamb )
    {
        ...
        if ( PVptr )
        {
            ...
            lambBr.refitMom.mass       = cand->composite().momentum().mass();
            lambBr.refitMom.pt         = cand->composite().momentum().pt(); 
            lambBr.refitMom.eta        = cand->composite().momentum().eta(); 
            lambBr.refitMom.phi        = cand->composite().momentum().phi(); 
                          
            lambBr.refitPos.x          = cand->composite().vertex().position().x(); 
            lambBr.refitPos.y          = cand->composite().vertex().position().y(); 
            lambBr.refitPos.z          = cand->composite().vertex().position().z(); 
            lambBr.refitPos.vtxprob    = TMath::Prob( cand->composite().vertex().chi2(), cand->composite().vertex().ndof() );


        lambBr.jpsiMom.mass      = cand_jpsi->composite().momentum().mass(); 
        lambBr.jpsiMom.pt        = cand_jpsi->composite().momentum().pt(); 
        lambBr.jpsiMom.eta       = cand_jpsi->composite().momentum().eta(); 
        lambBr.jpsiMom.phi       = cand_jpsi->composite().momentum().phi(); 

        lambBr.jpsiPos.x         = cand_jpsi->composite().vertex().position().x(); 
        lambBr.jpsiPos.y         = cand_jpsi->composite().vertex().position().y(); 
        lambBr.jpsiPos.z         = cand_jpsi->composite().vertex().position().z(); 
        lambBr.jpsiPos.vtxprob   = TMath::Prob( cand_jpsi->composite().vertex().chi2(), cand_jpsi->composite().vertex().ndof() );


        lambBr.lam0Mom.mass     = cand_lam0->composite().momentum().mass(); 
        lambBr.lam0Mom.pt       = cand_lam0->composite().momentum()->pt(); 
        lambBr.lam0Mom.eta      = cand_lam0->composite().momentum()->eta(); 
        lambBr.lam0Mom.phi      = cand_lam0->composite().momentum()->phi(); 

        lambBr.lam0Pos.x        = cand_lam0->composite().vertex()->position().x(); 
        lambBr.lam0Pos.y        = cand_lam0->composite().vertex()->position().y(); 
        lambBr.lam0Pos.z        = cand_lam0->composite().vertex()->position().z(); 
        lambBr.lam0Pos.vtxprob  = TMath::Prob( cand_lam0->composite().vertex()->chi2(), cand_lam0->composite().vertex()->ndof() );
    ...
    if ( writeLbToTkTk )
    {
        ...
        for ( unsigned int iLamb = 0; iLamb < nLamb; ++iLamb )
        {
            ...
            lbv1Br.refitMom.mass    = cand->composite().momentum().mass();
            lbv1Br.refitMom.pt      = cand->composite().momentum().pt();
            lbv1Br.refitMom.eta     = cand->composite().momentum().eta();
            lbv1Br.refitMom.phi     = cand->composite().momentum().phi();
            lbv1Br.refitPos.x       = cand->composite().vertex().position().x();
            lbv1Br.refitPos.y       = cand->composite().vertex().position().y();
            lbv1Br.refitPos.z       = cand->composite().vertex().position().z();
            lbv1Br.refitPos.vtxprob = TMath::Prob( cand->composite().vertex()->chi2(), cand->composite().vertex()->ndof() );


            lbv1Br.jpsiMom.mass      = cand_jpsi->composite().momentum().mass();
            lbv1Br.jpsiMom.pt        = cand_jpsi->composite().momentum().pt();
            lbv1Br.jpsiMom.eta       = cand_jpsi->composite().momentum().eta();
            lbv1Br.jpsiMom.phi       = cand_jpsi->composite().momentum().phi();
            lbv1Br.jpsiPos.x         = cand_jpsi->composite().vertex().position().x();
            lbv1Br.jpsiPos.y         = cand_jpsi->composite().vertex().position().y();
            lbv1Br.jpsiPos.z         = cand_jpsi->composite().vertex().position().z();
            lbv1Br.jpsiPos.vtxprob   = TMath::Prob( cand_jpsi->composite().vertex().chi2(), cand_jpsi->composite().vertex().ndof() );
...
void testProducer::setRecoParameters( const edm::ParameterSet& ps )
{
    ...
    while ( pIter != pIend )
    {
        ...
        if ( pv > -1.0e35 ) edm::LogVerbatim( "Configuration" )
                    << "testProducer::setRecoParameters: set " << pn
                    << " for " << name << " : "
    ...
    while ( fIter != fIend )
    {
        ...
        edm::LogVerbatim( "Configuration" )
                << "testProducer::setRecoParameters: set " << fn
                << " for " << name << " : "
                << ( parMap[rMap[name]][id] =
